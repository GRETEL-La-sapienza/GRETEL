...
class GAN(BaseGAN):
    ...
    def infinite_data_stream(self, loader):
        ...
        while True:
            for adj, label, node_features in loader:
                ...
                yield adj.to(self.device, dtype=torch.float32), label, node_features.to(self.device, dtype=torch.float32) #modificato aggiunto 
    ...
    def real_fit(self):
        ...
        for epoch in range(self.epochs):
            ...
            f_graph = f_graph.to(self.device, dtype=torch.float32)[:, None, :, :] #modificato aggiunto
            # generator data (fake batch)
            cf_graph, cf_node_features, _  = next(generator_loader)
            #cf_graph = cf_graph.double().to(self.device)[:,None,:,:]
            cf_graph = cf_graph.to(self.device, dtype=torch.float32)[:, None, :, :] #modificato aggiunto
    
            
            #cf_graph = self.generator(cf_graph).double()
            cf_graph = self.generator(cf_graph).to(self.device, dtype=torch.float32) #modificato aggiunto
            ...
            y_batch = torch.cat([torch.ones(self.batch_size,), torch.zeros(self.batch_size,)], dim=0).to(self.device, dtype=torch.float32) #modificato aggiunto
            #######################################################################
            ...
            loss = torch.mean(self.loss_fn(y_pred.squeeze(), y_batch) * torch.tensor(oracle_scores, dtype=torch.float32)) #modificato aggiunto
            ...
            cf_graph = cf_graph.to(self.device, dtype=torch.float32)[:, None, :, :] #modificato aggiunto
            y_fake = torch.ones((self.batch_size, 1), dtype=torch.float32).to(self.device) #modificato aggiunto
            #output = self.discriminator(self.generator(cf_graph).double())
            output = self.discriminator(self.generator(cf_graph)) #modificato aggiunto
            # calculate the loss
            #loss = self.loss_fn(output.double(), y_fake.double())
            loss = self.loss_fn(output, y_fake) #modificato aggiunto
            loss.backward()
...
class GAN(BaseGAN):
    ...
    def infinite_data_stream(self, loader):
        ...
        while True:
            for adj, label, node_features in loader:
                yield adj.float().to(self.device), label, node_features.float().to(self.device)
    ...
    def real_fit(self):
        ...
        for epoch in range(self.epochs):
            ...
            # discriminator data (real batch)
            f_graph, f_node_features, _ = next(discriminator_loader)
            f_graph = f_graph.float().to(self.device)[:, None, :, :]

            # generator data (fake batch)
            cf_graph, cf_node_features, _  = next(generator_loader)
            cf_graph = cf_graph.float().to(self.device)[:,None,:,:]
            
            cf_graph = self.generator(cf_graph).float()
            # get the real and fake labels
            y_batch = torch.cat([torch.ones(self.batch_size,), torch.zeros(self.batch_size,)], dim=0).to(self.device)
            #######################################################################
            # get the oracle's predictions
            ...
            fake_pred = self.discriminator(cf_graph)
            y_pred = torch.cat([real_pred, fake_pred])
            loss = torch.mean(self.loss_fn(y_pred.squeeze().float(), y_batch.float()) * torch.tensor(oracle_scores, dtype=torch.float))
            D_losses.append(loss.item())
            loss.backward()
            self.discriminator_optimizer.step()
            #######################################################################
            ...
            cf_graph = cf_graph.float().to(self.device)[:,None,:,:]
            y_fake = torch.ones((self.batch_size, 1)).to(self.device)
            output = self.discriminator(self.generator(cf_graph).float())
            # calculate the loss
            loss = self.loss_fn(output.float(), y_fake.float())
            loss.backward()
            G_losses.append(loss.item())
            self.generator_optimizer.step()
    ...
    def retake_batch(self, graph: torch.Tensor, node_features: torch.Tensor, counterfactual=True):
        ...
        for i in range(len(graph)):
            instances.append(GraphInstance(id="dummy",
                                           ...
                                           data=graph[i].squeeze().cpu().numpy(),
                                           node_features=node_features[i].cpu().numpy()))
                                           #data=graph[i].squeeze().numpy(),
                                           #node_features=node_features[i].numpy()))
                                           #data=graph[i].squeeze(),  # Maintain as tensor, no .cpu() or .numpy()
                                           #node_features=node_features[i] ))
            #print(f"----passa di qui da cpu -----")
        return instances    
